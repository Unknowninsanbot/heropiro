...
def send_welcome(message):
    ...
    time.sleep(0.3)
    bot.edit_message_text("<i>🔐 Establishing secure connection... [██░░░░░░░░] 20%</i>", chat_id, msg.message_id, parse_mode='HTML')
    time.sleep(0.3)
    bot.edit_message_text("<i>⚡ Loading modules... [██████░░░░] 60%</i>", chat_id, msg.message_id, parse_mode='HTML')
    time.sleep(0.3)
    bot.edit_message_text("<i>✅ System ready. [██████████] 100%</i>", chat_id, msg.message_id, parse_mode='HTML')
    time.sleep(0.3)
...
def site_menu_callback(call):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🗑️ Clear All", callback_data="site_clear"),
        types.InlineKeyboardButton("🔙 Back", callback_data="back_to_start")
    )
    bot.edit_message_text(
        "<b>🌐 Personal Site Manager</b>\n\n"
        "Add your own Shopify sites to use in mass checks.",
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        parse_mode='HTML',
        reply_markup=markup
...
@bot.callback_query_handler(func=lambda call: call.data == "site_add_prompt")
def site_add_prompt(call):
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id,
        "🔗 <b>Send me the Shopify site URL:</b>\nExample: <code>https://store.com</code>",
        parse_mode='HTML')
    bot.register_next_step_handler(call.message, process_add_my_site)


def process_add_my_site(message):
    handle_add_my_site(message)


@bot.callback_query_handler(func=lambda call: call.data == "site_view")
def site_view_callback(call):
    user_id = call.from_user.id
    sites = get_user_sites(user_id)
    if not sites:
        text = "❌ You have no personal sites."
    else:
        text = f"<b>🌐 Your Sites ({len(sites)}):</b>\n\n" + "\n".join([s['url'] for s in sites])
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id, text, parse_mode='HTML')


@bot.callback_query_handler(func=lambda call: call.data == "site_clear")
def site_clear_callback(call):
    user_id = call.from_user.id
    save_user_sites_list(user_id, [])
    bot.answer_callback_query(call.id, "✅ All your sites cleared.")
...
<b>💳 CARDS</b>
Use the <b>Single Check</b> or <b>Mass Check</b> buttons in the menu.

<b>👤 PERSONAL</b>
<code>/addmysite</code> – add your own Shopify site
